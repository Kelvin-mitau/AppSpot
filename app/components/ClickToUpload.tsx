import "../styles/File-upload-input.css"
...
const ClickToUpload: React.FC<props> = ({ id, handleFileUpload, title, disabled, acceptedFormat, name, styles }) => {
    return (
        <div className={`File-upload-input ${styles ? styles : ""}`} >
            <label className="custum-file-upload" htmlFor={id}>
                ...
                <input type="file" accept={acceptedFormat} name={name ? name : ""} id={id} onChange={({ target }) => handleFileUpload(target)} disabled={disabled} />
            </label>
        </div>
