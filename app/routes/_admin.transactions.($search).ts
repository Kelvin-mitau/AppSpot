...
export const action:ActionFunction = async ({request}) => {
    try {
        ...
        //Send money to user
        const {_id,price,seller,title} = await Product.findById(productID).select(["price","_id","title","seller"]).populate("seller")
        const ppEmail = seller.paymentDetails.paypalEmail
        const phoneNumber = seller.businessDetails.businessPhoneNumber
        const countryCode = seller.businessDetails.businessCountryCode
