    ...
    return (
        <>
            ...
            <div className='flex flex-row'>
                <div className='hidden sm:block p-2 bg-[#1a1c89] bg-opacity-20 rounded my-2'>
                    <p className='text-lg'>Categories</p>
                    <div className='flex flex-col gap-2'>
                        {productCategories.map(({ title, link }, index) => <Link key={index} to={link}>
                            <span className='bg-white block px-1 rounded bg-opacity-15'>{title}</span></Link>)}
                    </div>
                </div>
                <div className='w-[80%]'>
                    ...
                    <div className='grid grid-cols-3 gap-1 mx-2'>
                        {
                            products.map(({ _id, customerReviews, title, description, productURL, pricingModel, price, screenshots }) => {
                                return (
                                    <Link to={`/product/${_id}`}>
                                        <div key={_id} className='Product-card h-full mx-2 flex flex-col justify-between p-1 text-white' >
                                            ...
                                            <div className='mx-1'>
                                                ...
                                                {productURL && <a className='underline' href={productURL} target='_self'>Preview</a>}
